...
CSV_FINAL = PROJECT / "results" / "empirical_calibration_stage2" / "raw" / "stage2_gate1_central.csv"
CSV_STAGING = Path(r"C:\Users\deepa\.gemini\antigravity-ide\scratch\stage2_gate1_central.csv")
...
def run_sim(protocol, speed, seed):
    cmd = [
        str(EXE),
        f"--protocol={protocol}",
        f"--numNodes=30",
...
def main():
    ...
    seeds = list(range(1, 31)) # 30 matched evaluation seeds
    
    total = len(protocols) * len(speeds) * len(seeds)
    print(f"============================================================")
    print(f" Stage 2 Gate 1: Central Matched PPR vs RVC (Empirical Calib)")
